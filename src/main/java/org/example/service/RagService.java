...
public class RagService {
    ...
    public void queryStream(String question, List<Map<String, String>> history, StreamCallback callback) {
        try {
            ...
            // 1. 混合检索（向量 + BM25 + RRF 融合），返回所有 RRF 候选
            List<VectorSearchService.SearchResult> searchResults =
                hybridSearchService.hybridSearch(question, topK);

            // 2. 对所有 RRF 候选结果进行重排（语义级别二次排序）
            List<VectorSearchService.SearchResult> rerankedResults =
                dashScopeRerankService.rerank(question, searchResults);

            // 3. 从重排结果中截取 topK 条用于最终输出
