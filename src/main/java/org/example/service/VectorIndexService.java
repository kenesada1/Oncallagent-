...
public class VectorIndexService {
    ...
    public void indexSingleFile(String filePath) throws Exception {
        ...
        for (int i = 0; i < chunks.size(); i++) {
            ...
            try {
                ...
                // === 同步写入 Elasticsearch（双写）===
                insertToElasticsearch(chunk, source, metadata, chunk.getChunkIndex());
    ...
    private void deleteExistingData(String filePath) {
        try {
            ...
            // === 同步删除 Elasticsearch 数据（双删）===
            try {
                keywordSearchService.deleteBySource(normalizedPath);
                logger.info("✓ 已从 ES 删除旧数据: {}", normalizedPath);
            } catch (Exception e) {
                logger.warn("从 ES 删除旧数据失败（不影响主流程）: {}", e.getMessage());
            }
     ...
     * 同步写入 Elasticsearch（双写）
     */
    private void insertToElasticsearch(DocumentChunk chunk, String source,
                                        Map<String, Object> metadata, int chunkIndex) {
        String id = UUID.nameUUIDFromBytes((source + "_" + chunkIndex).getBytes()).toString();

        try {
            org.example.entity.ChunkDocument esDoc = new org.example.entity.ChunkDocument();
            esDoc.setId(id);
            esDoc.setContent(chunk.getContent());
            esDoc.setSource(source);
            esDoc.setChunkIndex(chunkIndex);
            esDoc.setTitle(chunk.getTitle());
            esDoc.setMetadata(new com.google.gson.Gson().toJson(metadata));

            keywordSearchService.saveDocument(esDoc);
            logger.debug("ES 文档写入成功: id={}", id);

        } catch (Exception e) {
            throw new RuntimeException("写入 Elasticsearch 失败: id=" + id, e);
        }
    }
