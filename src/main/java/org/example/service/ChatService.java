...
public class ChatService {
    ...
    public void logAvailableTools() {
        logger.info("========== MCP 工具诊断开始 ==========");
        logger.info("ToolCallbackProvider 实例: {}", tools);
        logger.info("QueryLogsTools 实例: {}", queryLogsTools);

        ToolCallback[] toolCallbacks = tools.getToolCallbacks();
        logger.info("MCP 工具数量: {}", toolCallbacks.length);

        if (toolCallbacks.length == 0) {
            logger.warn("⚠️ 警告：MCP 未提供任何工具！可能的原因：");
            logger.warn("  1. MCP SSE 连接失败或未建立");
            logger.warn("  2. 腾讯云 MCP 服务未正确配置");
            logger.warn("  3. MCP 服务器未暴露任何工具");
        } else {
            logger.info("可用工具列表:");
        ...
        // 检查 QueryLogsTools 是否被使用（Mock模式）
        Object[] methodTools = buildMethodToolsArray();
        logger.info("本地方法工具数量: {}", methodTools.length);
        for (Object tool : methodTools) {
            logger.info(">>> 本地工具: {}", tool.getClass().getSimpleName());
        }

        // 检查是否有 QueryLogsTools 在数组中
        boolean hasQueryLogs = false;
        for (Object tool : methodTools) {
            if (tool instanceof QueryLogsTools) {
                hasQueryLogs = true;
                break;
            }
        }
        logger.info("QueryLogsTools 是否在本地工具中: {}", hasQueryLogs);
        logger.info("========== MCP 工具诊断结束 ==========");
    }
