...
import lombok.Data;
import okhttp3.*;
import org.example.util.Tc3Signature;
 ...
 *
 * 注意：此组件仅在 cls.mock-enabled=true 时创建
 * 真实 CLS 调用通过 MCP 服务器提供
 */
@Component
@ConditionalOnProperty(name = "cls.mock-enabled", havingValue = "true", matchIfMissing = false)
public class QueryLogsTools {
    ...
    @Value("${cls.enabled:false}")
    private boolean clsEnabled;
    ...
    @Value("${cls.endpoint:cls.ap-guangzhou.tencentcloudapi.com}")
    private String endpoint;

    @Value("${cls.topics.system-metrics:}")
    private String systemMetricsTopicId;

    @Value("${cls.topics.application-logs:}")
    private String applicationLogsTopicId;

    @Value("${cls.topics.database-slow-query:}")
    private String databaseSlowQueryTopicId;

    @Value("${cls.topics.system-events:}")
    ...
    public void init() {
        ...
        if (clsEnabled && !secretId.isEmpty() && !secretKey.isEmpty()) {
            logger.info("✅ QueryLogsTools 初始化成功 - 真实 CLS 模式, Region: {}, Endpoint: {}", region, endpoint);
        } else if (mockEnabled) {
            logger.info("✅ QueryLogsTools 初始化成功 - Mock 模式");
        } else {
            logger.warn("⚠️ QueryLogsTools 初始化警告 - 未启用真实 CLS (cls.enabled=false 或 secret 未配置) 且 mock-enabled=false");
        }
    }

    /**
     * 根据日志主题名获取对应的 TopicId
     */
    private String getTopicId(String logTopic) {
        if (logTopic == null) return "";

        return switch (logTopic.toLowerCase()) {
            ...
            default -> logTopic; // 如果不是已知主题名，认为传入的就是 TopicId
        };
    }
    ...
    public String queryLogs(
        ...
        try {
            ...
            } else if (clsEnabled && !secretId.isEmpty() && !secretKey.isEmpty()) {
                // 真实模式：调用腾讯云 CLS API
                logEntries = fetchClsLogs(region, logTopic, safeQuery, actualLimit);
                logger.info("使用真实 CLS 数据，返回 {} 条日志", logEntries.size());
            } else {
                // 未配置真实模式或 mock 模式
                return buildErrorResponse("CLS 未正确配置：请在 application.yml 中配置 cls.secret-id 和 cls.secret-key，或启用 cls.mock-enabled=true");
            }
     ...
     * 从腾讯云 CLS API 获取日志
     */
    private List<LogEntry> fetchClsLogs(String region, String logTopic, String query, int limit) {
        List<LogEntry> logEntries = new ArrayList<>();

        try {
            String topicId = getTopicId(logTopic);
            if (topicId == null || topicId.isEmpty()) {
                logger.error("未配置日志主题 ID: {}", logTopic);
                return logEntries;
            }
            ...
            if (query != null && !query.isBlank()) {
                ...
                queryObj.put("SyntaxRule", "lucene");
            ...
            // 生成签名头
            Map<String, String> headers = Tc3Signature.buildHeaders(secretId, secretKey, jsonPayload);
            ...
            try (Response response = httpClient.newCall(request).execute()) {
                if (!response.isSuccessful()) {
                    logger.error("CLS API 请求失败: HTTP {}", response.code());
                    return logEntries;
                }

                String responseBody = response.body() != null ? response.body().string() : "";
                logger.debug("CLS API 响应: {}", responseBody);
                ...
                if ("0".equals(String.valueOf(result.get("ErrorCode")))) {
                    ...
                    if (data != null && data.containsKey("Logs")) {
                        ...
                        for (Map<String, Object> log : logs) {
                            ...
                            entry.setTimestamp(String.valueOf(log.get("Time")));
                            entry.setLevel(String.valueOf(log.getOrDefault("Level", "INFO")));
                            entry.setService(String.valueOf(log.getOrDefault("Source", "unknown")));
                            entry.setMessage(String.valueOf(log.getOrDefault("Message", "")));

                            // 解析 metrics
                            Map<String, String> metrics = new HashMap<>();
                            if (log.containsKey("Key")) {
                                @SuppressWarnings("unchecked")
                                List<Map<String, Object>> keys = (List<Map<String, Object>>) log.get("Key");
                                for (Map<String, Object> key : keys) {
                                    String k = String.valueOf(key.get("Key"));
                                    String v = String.valueOf(key.get("Value"));
                                    metrics.put(k, v);
                                }
                            }
                    ...
                    logger.error("CLS API 返回错误: {}", result.get("ErrorMessage"));
                }
            }

        } catch (Exception e) {
            logger.error("调用 CLS API 失败", e);
        }
