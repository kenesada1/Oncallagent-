...
public class ChatController {
    ...
    public ResponseEntity<ApiResponse<ChatResponse>> chat(@RequestBody ChatRequest request) {
        try {
            ...
            // 意图路由
            RouteResult routeResult = intentRouter.route(request.getQuestion());
            logger.info("意图路由结果: intent={}, confidence={}, uncertain={}",
                    routeResult.getIntent(), routeResult.getConfidence(), routeResult.isUncertain());

            // 只要是 KNOWLEDGE 就走 RAG，不看 uncertain 标志
            if (routeResult.getIntent() == IntentEnum.KNOWLEDGE) {
                logger.info("路由到 RAG 流程，执行知识库检索...");
                return handleRagRequest(request, routeResult);
            }

            // CODE / REALTIME / SYSTEM / CHITCHAT 走 ReactAgent
            logger.info("路由到 ReactAgent 流程（工具调用）");
            return handleReactAgentRequest(request, routeResult);
     ...
     * 处理 RAG 请求
     */
    private ResponseEntity<ApiResponse<ChatResponse>> handleRagRequest(ChatRequest request, RouteResult routeResult) {
        try {
            ...
            ragService.queryStream(request.getQuestion(), history, callback);
            ...
            if (callback.error != null) {
                ...
                return ResponseEntity.ok(ApiResponse.success(ChatResponse.error("RAG 执行失败: " + callback.error.getMessage())));
            }

            logger.info("RAG 执行完成，答案长度: {}", fullAnswer.length());
            session.addMessage(request.getQuestion(), fullAnswer.toString());
            return ResponseEntity.ok(ApiResponse.success(ChatResponse.success(fullAnswer.toString())));

        } catch (Exception e) {
            logger.error("RAG 请求处理失败", e);
            return ResponseEntity.ok(ApiResponse.success(ChatResponse.error(e.getMessage())));
        }
    }
     ...
     * 处理 ReactAgent 请求
     */
    private ResponseEntity<ApiResponse<ChatResponse>> handleReactAgentRequest(ChatRequest request, RouteResult routeResult) {
        try {
            SessionInfo session = getOrCreateSession(request.getId());
            List<Map<String, String>> history = session.getHistory();
            ...
            logger.error("ReactAgent 请求处理失败", e);
            return ResponseEntity.ok(ApiResponse.success(ChatResponse.error(e.getMessage())));
        }
    }
     ...
     * 处理 RAG 流式请求
     */
    private void handleRagStreamRequest(ChatRequest request, RouteResult routeResult, SseEmitter emitter) {
        try {
            SessionInfo session = getOrCreateSession(request.getId());
            List<Map<String, String>> history = session.getHistory();
            ...
            if (callback.error != null) {
                logger.error("RAG 流式执行失败", callback.error);
                emitter.send(SseEmitter.event()
                        .name("message")
                        .data(SseMessage.error("RAG 执行失败: " + callback.error.getMessage()), MediaType.APPLICATION_JSON));
            } else {
                logger.info("RAG 流式执行完成，答案长度: {}", fullAnswer.length());
                // 发送完整答案
                emitter.send(SseEmitter.event()
                        .name("message")
                        .data(SseMessage.content(fullAnswer.toString()), MediaType.APPLICATION_JSON));
            }
            ...
            logger.error("RAG 流式请求发送失败", e);
            emitter.completeWithError(e);
        } catch (Exception e) {
            logger.error("RAG 流式请求处理失败", e);
            try {
                emitter.send(SseEmitter.event()
                        .name("message")
    ...
    public SseEmitter chatStream(@RequestBody ChatRequest request) {
        ...
        executor.execute(() -> {
            try {
                ...
                // 只要是 KNOWLEDGE 就走 RAG 流式
                if (routeResult.getIntent() == IntentEnum.KNOWLEDGE) {
                    logger.info("路由到 RAG 流式流程");
                    handleRagStreamRequest(request, routeResult, emitter);
                    return;
                }
